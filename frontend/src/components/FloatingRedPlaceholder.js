...
const FloatingRedPlaceholder = () => {
  ...
  const stateRef = useRef({
    ...
    midScroll: 0,
    endScroll: 0,
    originW: ANIMATION_DIMENSIONS.ORIGIN_WIDTH,
    originH: ANIMATION_DIMENSIONS.ORIGIN_HEIGHT,
    midW: ANIMATION_DIMENSIONS.MID_WIDTH,
    midH: ANIMATION_DIMENSIONS.MID_HEIGHT,
    targetW: ANIMATION_DIMENSIONS.TARGET_WIDTH,
    targetH: ANIMATION_DIMENSIONS.TARGET_HEIGHT,
    targetEl: null,
  });
  ...
  const measure = () => {
    ...
    s.originW = Math.max(SIZE_CONSTRAINTS.MIN_WIDTH, Math.min(SIZE_CONSTRAINTS.MAX_WIDTH, ro.width * SIZE_CONSTRAINTS.WIDTH_SCALE));
    s.originH = Math.round(s.originW * SIZE_CONSTRAINTS.HEIGHT_RATIO);
    const offsetRight = SPACING.OFFSET_RIGHT; // start centered under the card
    const gapBelow = SPACING.GAP_BELOW; // smaller gap so it visually attaches to the card bottom
